...
select PROBLEM_ID ,count(PROBLEM_ID) as sum from User join Solve on User.ID = Solve.USER_ID where ID in (
...
select ID from User where skhurank = (select skhurank from User where ID='q9922000')-1
union
select ID from User where skhurank = (select skhurank from User where ID='q9922000')-2) group by PROBLEM_ID having count(PROBLEM_ID)>1 order by count(PROBLEM_ID) desc;

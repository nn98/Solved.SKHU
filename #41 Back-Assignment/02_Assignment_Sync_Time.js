...
async function run() {
    ...
    next().then(rurl => {
        puppeteer.launch({ headless: true }).then(async browser => {
            ...
            let smtime = await page.$eval("a.real-time-update show-date ", e => e.outerHTML);

            console.log(rurl, '\n', rurl.split('&user_id=')[1].split('&language_id')[0],smtime);
            console.log("\n");
